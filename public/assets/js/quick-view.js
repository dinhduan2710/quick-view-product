( function( $, settings, ids, styles ) {
	...
	var tmQuickView = {
		css: {
			...
			closeBtn: '.quick-view-popup__close',
			overlay: '.quick-view-popup__overlay',
		...
		prepareObjects: function() {
			...
			if ( null === tmQuickView.content ) {
				tmQuickView.content = $( '.quick-view-popup__content', tmQuickView.popup );
			}
		},
		...
		initPopup: function( event ) {
			...
			$.ajax({
				...
				$( document ).trigger( 'woo-quick-view-on-show' );
			});
